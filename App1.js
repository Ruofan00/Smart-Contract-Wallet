...
class App extends Component {
  ...
  async transfer(recipient, amount) {
    ...
    console.log(window.web3.utils.isAddress(contractRecipient));
    console.log(window.web3.utils.isAddress(this.state.address));
    const transferResult = await this.state.myMessenger.methods.transfer(recipient,amount).send({from:this.state.address}, function(err, res){
      if(err){
 ...
 async withDraw(){
    ...
    // })
    console.log("1");
   this.state.myMessenger.methods.withdraw().send({from:this.state.address});
 }
async deposit(){
