...
class App extends Component {
  ...
  async loadBlockchainData() {
   ...
   // var messenger = "0xC99e38A8FC4B84FC675C49850e0C055dfB33DBB2";
    ...
    var messenger = "0x0ae87FBEeB9C8129eb7830CDf92eF771a1EE659A"
    ...
    const transactions = await userFunction.getPastEvents('Transfer', { fromBlock: 0, toBlock: 'latest', filter: { from: this.state.address } })
  ...
  async deposit() {
     var deposit = await this.state.myMessenger.methods.deposit().send({
  ...
  async transfer(recipient, _amount) {
    ...
    const transferResult = await this.state.myMessenger.methods.debit(this.state.recipient,this.state.trans_amount)
    .send({
      from: this.state.account,
    });
 ...
 async withdraw(){
   this.state.myMessenger.methods.withdraw().send(
      {from:this.state.account}
    );
                    ...
                    { this.state.transactions.map((tx, key) => {
                      return (
                        <tr key={key} >
                          ...
                          <td>{tx.returnValues._value} Wei</td>
                          <td>Recover</td>
                        </tr>
                      )
                    }) }
